...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen pb-24 bg-background text-text-main font-sans selection:bg-primary selection:text-black">
      {/* 1. Top Header */}
      <header className="sticky top-0 z-50 glass border-b border-white/5 px-4 h-16 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="text-2xl font-black italic tracking-tighter text-primary flex items-center gap-1">
            LIVO
            <div className="w-1.5 h-1.5 bg-accent rounded-full animate-pulse mt-2" />
          </div>
        </div>
      ...
      <main className="max-w-md mx-auto px-4 pt-4 space-y-6">
        ...
        <div className="glass rounded-xl p-3 flex items-center gap-3 overflow-hidden">
          <div className="bg-primary text-black text-[10px] font-black px-2 py-0.5 rounded italic">HOT</div>
        ...
        <section className="card-premium p-4">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xs font-black text-primary tracking-widest uppercase">Quick Actions</h3>
            <span className="text-[10px] text-text-muted font-bold cursor-pointer hover:text-primary transition-colors">SEE ALL →</span>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={i} className="card-premium p-3 text-center border-white/5">
              <div className="text-xl font-black italic text-primary leading-none tracking-tighter">{stat.value}</div>
        ...
        <section className="card-premium overflow-hidden">
          <div className="p-4 border-b border-white/5 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] font-bold text-secondary">LIVE UPDATING</span>
            </div>
          </div>
          <div className="divide-y divide-white/5">
            {[
              { user: "***882", amount: "৳15,68,523", icon: "🥇", color: "text-primary" },
              ...
              <div key={i} className="px-4 py-3 flex items-center justify-between hover:bg-white/5 transition-colors">
                ...
                <div className="text-secondary font-black italic tracking-tighter">{winner.amount}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="card-premium p-4 border-l-4 border-primary group cursor-pointer">
            <div className="text-[10px] font-black text-text-muted uppercase mb-1">Welcome</div>
            <div className="text-2xl font-black italic text-primary group-hover:scale-105 transition-transform origin-left tracking-tighter">100% BONUS</div>
          </div>
          <div className="card-premium p-4 border-l-4 border-secondary group cursor-pointer">
            <div className="text-[10px] font-black text-text-muted uppercase mb-1">Daily</div>
            <div className="text-2xl font-black italic text-secondary group-hover:scale-105 transition-transform origin-left tracking-tighter">10% CASHBACK</div>
          </div>
        </div>

        {/* Footer info */}
        <footer className="pt-8 pb-4 text-center space-y-6">
          <div className="text-3xl font-black italic tracking-tighter text-white/20">LIVO</div>
          ...
          <div className="flex justify-center gap-4">
            <div className="glass px-3 py-1 rounded-full text-accent text-[10px] font-black">🔞 18+</div>
      ...
      {/* Bottom Nav */}
      <nav className="fixed bottom-0 left-0 right-0 z-50 glass border-t border-white/5 px-4 h-20 flex items-center justify-around max-w-md mx-auto">
        ...
        <Link href="/" className="relative -mt-10 group">
          <div className="w-16 h-16 rounded-full bg-gold-gradient p-1 shadow-[0_0_20px_rgba(251,191,36,0.5)] group-active:scale-95 transition-transform">
            <div className="w-full h-full rounded-full bg-background flex items-center justify-center">
              <div className="text-primary font-black italic tracking-tighter">HOME</div>
            </div>
          </div>
        </Link>
...
function BottomNavLink({ icon, label, active = false }: { icon: React.ReactNode, label: string, active?: boolean }) {
  return (
    <div className={`flex flex-col items-center gap-1.5 cursor-pointer transition-colors ${active ? 'text-primary' : 'text-text-muted'}`}>
      {icon}
