...
const gameHandlers = {
  slots: (betAmount) => {
    const symbols = ["ð", "ð", "ð", "ð", "ð", "ð", "7ï¸â£", "â­", "ð", "ð"];
...
router.get('/play', isAuth, (req, res) => {
  ...
  if (!supportedGames[gameSlug]) {
    req.flash('error', 'à¦à§à¦®à¦à¦¿ à¦ªà¦¾à¦à¦¯à¦¼à¦¾ à¦¯à¦¾à¦¯à¦¼à¦¨à¦¿');
    return res.redirect('/');
  }
...
router.post('/play', isAuth, async (req, res) => {
  ...
  if (isNaN(betAmount) || betAmount <= 0) return res.status(400).json({ success: false, message: 'à¦¸à¦ à¦¿à¦ à¦ªà¦°à¦¿à¦®à¦¾à¦£ à¦¦à¦¿à¦¨' });
  ...
  try {
    ...
    if (userResult.rows[0].coins < betAmount) {
      ...
      return res.status(400).json({ success: false, message: 'à¦ªà¦°à§à¦¯à¦¾à¦ªà§à¦¤ à¦¬à§à¦¯à¦¾à¦²à§à¦¨à§à¦¸ à¦¨à§à¦' });
    }
    ...
    if (['aviator', 'crash-game'].includes(gameSlug)) {
      ...
      return res.json({ success: true, message: 'à¦à§à¦® à¦¶à§à¦°à§ à¦¹à¦¯à¦¼à§à¦à§' });
    }
    ...
    await client.query('INSERT INTO coin_transactions (user_id, amount, type, description) VALUES ($1, $2, $3, $4)', 
                       [userId, netChange, 'game_play', `${supportedGames[gameSlug] || gameSlug} à¦à§à¦®`]);
    await client.query('COMMIT');
...
router.post('/cashout', isAuth, async (req, res) => {
  ...
  if (!state || state.game !== gameSlug) {
    return res.status(400).json({ success: false, message: 'à¦à§à¦¨à§ à¦à¦²à¦®à¦¾à¦¨ à¦à§à¦® à¦¨à§à¦' });
  }

  const cashMultiplier = parseFloat(multiplier);
  if (isNaN(cashMultiplier) || cashMultiplier < 1) {
    return res.status(400).json({ success: false, message: 'à¦à¦à¦¾à¦°à§à¦¯à¦à¦° à¦®à¦¾à¦²à§à¦à¦¿à¦ªà§à¦²à¦¾à¦¯à¦¼à¦¾à¦°' });
  }
  ...
  if (cashMultiplier > state.crashPoint) {
    ...
    return res.json({
      ...
      message: `à¦à¦¡à¦¼à§à¦à¦¾à¦¹à¦¾à¦ ${state.crashPoint}x-à¦ à¦à§à¦°à§à¦¯à¦¾à¦¶ à¦à¦°à§à¦à§!`
    });
  }
  ...
  try {
    ...
    await client.query(
      ...
      [userId, winAmount, 'game_win', `${supportedGames[gameSlug] || gameSlug} à¦à§à¦¯à¦¾à¦¶à¦à¦à¦ ${cashMultiplier}x`]
    );
    await client.query('COMMIT');
